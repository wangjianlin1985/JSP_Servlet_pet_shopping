...
public class RegisterInServlet extends HttpServlet
{
    protected void service(final HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
        ...
        final String courty = req.getParameter("courty");
        final String[] likes = req.getParameterValues("like");
        String like = "";
        if (likes != null) {
            String[] array;
            for (int length = (array = likes).length, i = 0; i < length; ++i) {
                final String temp = array[i];
                like = String.valueOf(like) + temp + ",";
            }
        }
        final String veryCode = req.getParameter("veryCode");
        final HttpSession session = req.getSession();
        final String sysCode = (String)session.getAttribute("syscode");
        final EASYBUY_USER u = new EASYBUY_USER(username, name, rePassWord, sex, year, null, email, mobile, address, courty, like, 1);
